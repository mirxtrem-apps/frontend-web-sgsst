...
import { ItemEstandar } from 'src/app/interfaces/response/items_estantdar_response.interface';
...
@Component({
  selector: 'app-items-estandar-details',
  templateUrl: './items-estandar-details.component.html',
  styleUrls: ['./items-estandar-details.component.scss'],
})
export class ItemsEstandarDetailsComponent implements OnInit {
  constructor(
  ...
  ngOnInit(): void {
    this.params.params.subscribe(({ id }) => {
      this.sstService
        .getCiclos()
        .subscribe((resp) => (this.ciclos = resp.data));
      this.sstService
        .getItemDelEstandarById(id)
        .subscribe((resp) => (this.item = resp.data));
    });
  }
