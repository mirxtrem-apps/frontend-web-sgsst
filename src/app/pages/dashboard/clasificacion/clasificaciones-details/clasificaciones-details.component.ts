...
@Component({
  selector: 'app-clasificaciones-details',
  templateUrl: './clasificaciones-details.component.html',
  styleUrls: ['./clasificaciones-details.component.scss'],
})
export class ClasificacionesDetailsComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.request.params.subscribe(({ id }) => {
      this.clasificacionId = id;
      this.clasificacionService.getClasificacionPorId(id)
        .subscribe((resp) => {
          this.clasificacion = resp.data;
          this.numEmpleados = this.clasificacion.num_empleados!;
          this.nivelRiesgo = this.clasificacion.nivel_riesgo!;
  ...
  guardar(formulario: NgForm) {
    ...
    if (formulario.valid) {
      ...
      this.clasificacionService
        .updateClasificacion(this.clasificacionId, this.clasificacion)
        .subscribe(
          (resp) => {
            Swal.close();
            Swal.fire({
              icon: 'success',
              title: 'Clasificación actualizada éxitosamente',
